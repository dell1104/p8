...
from utils import (
    crear_estructura_caso, calcular_hash_sha256, clasificar_tipo_archivo,
    generar_nombre_archivo_seguro, guardar_hash_archivo, log_evento,
    validar_archivo_seguro, sanitizar_input_usuario, validar_expediente, log_seguridad
)
...
def crear_caso():
    ...
    try:
        ...
        es_valido_exp, mensaje_exp = validar_expediente(expediente_completo)
        if not es_valido_exp:
            log_seguridad('INVALID_EXPEDIENTE', f'Expediente inválido: {expediente_completo}', 
                         request.remote_addr, request.headers.get('User-Agent'))
            return jsonify({'success': False, 'error': mensaje_exp}), 400
        
        # Sanitizar inputs del usuario
        data['origen_solicitud'] = sanitizar_input_usuario(data.get('origen_solicitud', ''), 200)
        data['tipo_requerimiento'] = sanitizar_input_usuario(data.get('tipo_requerimiento', ''), 100)
        data['observaciones'] = sanitizar_input_usuario(data.get('observaciones', ''), 1000)
        ...
        print(f"🆔 Caso creado con ID: {nuevo_caso.id}")  # Debug log

        carpeta_caso = crear_estructura_caso(sesion_id, nuevo_caso.expediente, nuevo_caso.origen_solicitud)
...
def subir_archivo_caso():
    ...
    try:
        ...
        if 'archivo' not in request.files:
            log_seguridad('FILE_UPLOAD_ATTEMPT', 'Intento de subida sin archivo', 
                         request.remote_addr, request.headers.get('User-Agent'))
        ...
        if not sesion_id:
            log_seguridad('FILE_UPLOAD_ATTEMPT', 'Intento de subida sin sesion_id', 
                         request.remote_addr, request.headers.get('User-Agent'))
            return jsonify({'success': False, 'error': 'ID de sesión requerido'}), 400
        
        # Validar archivo con las nuevas funciones de seguridad
        es_valido, mensaje_error, tipo_mime = validar_archivo_seguro(archivo, max_size_mb=100)
        if not es_valido:
            log_seguridad('FILE_UPLOAD_REJECTED', f'Archivo rechazado: {mensaje_error}', 
                         request.remote_addr, request.headers.get('User-Agent'))
        ...
        if not caso or caso.usuario_id != current_user.id:
            log_seguridad('FILE_UPLOAD_UNAUTHORIZED', f'Intento de subida a caso no autorizado: {sesion_id}', 
                         request.remote_addr, request.headers.get('User-Agent'))
        ...
        nombre_archivo = generar_nombre_archivo_seguro(nombre_original)
        ...
        guardar_hash_archivo(ruta_archivo, hash_sha256, os.path.join(Config.BASE_UPLOAD_FOLDER, sesion_id, 'evidencia_en_proceso'))
...
def casos_completos():
    ...
    try:
        ...
        print(f"📊 API casos_completos: Devolviendo {len(casos_data)} casos")
        return jsonify({"success": True, "casos": casos_data})
